...
// 1. Convert 1.4 CBT_22 SIT UP.html
{
  const file = '1.4 CBT_22 SIT UP.html';
  ...
  if (m) {
    ...
    const normalized = raw.map((q, idx) => ({
      ...
      a: q.a,
      exp: q.exp || '',
      src: 'Bank Soal Blok 1.4 FK UNSOED'
    }));
    fs.writeFileSync(path.join(dir, file), getCbtTemplate('CBT: Sit Up Edition Blok 1.4', '1.4', 'Muskuloskeletal Detail', JSON.stringify(normalized, null, 2)));
    console.log(`Migrated ${file} -> ${normalized.length} questions`);
  }
}

// 2. Convert 1.4 CBT_23 PULL UP.html
{
  const file = '1.4 CBT_23 PULL UP.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  const m = content.match(/const questions\s*=\s*(\[[\s\S]*?\]);/);
  if (m) {
    const raw = eval(m[1]);
    ...
    fs.writeFileSync(path.join(dir, file), getCbtTemplate('CBT: Pull Up Edition Blok 1.4', '1.4', 'Support & Movement Revision', JSON.stringify(normalized, null, 2)));
    console.log(`Migrated ${file} -> ${normalized.length} questions`);
  }
}

// 3. Convert 1.4 CBT_24 SCOTT JUMP.html
{
  const file = '1.4 CBT_24 SCOTT JUMP.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  ...
  if (m) {
    ...
    const normalized = raw.map((q, idx) => {
      ...
      const ansKey = q.a || 'A';
      const ansIdx = ansKey.charCodeAt(0) - 65;
      return {
        id: idx + 1,
        q: q.q,
        o: opts,
        a: ansIdx >= 0 && ansIdx < opts.length ? ansIdx : 0,
        exp: q.exp || '',
        src: 'Bank Soal Blok 1.4 FK UNSOED'
      };
    });
    fs.writeFileSync(path.join(dir, file), getCbtTemplate('CBT: Scott Jump Edition Blok 1.4', '1.4', 'Support & Movement High-Yield', JSON.stringify(normalized, null, 2)));
    console.log(`Migrated ${file} -> ${normalized.length} questions`);
  }
}

// 4. Convert 1.4 CBT_Histologi Lecture.html
{
  const file = '1.4 CBT_Histologi Lecture.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  const m = content.match(/const questionsDB\s*=\s*(\[[\s\S]*?\]);/);
  if (m) {
    const raw = eval(m[1]);
    const normalized = raw.map((q, idx) => ({
      id: idx + 1,
      ...
      a: q.a,
      exp: q.rationale || '',
      src: 'Histologi Otot, Tulang & Integumen FK UNSOED'
    }));
    fs.writeFileSync(path.join(dir, file), getCbtTemplate('CBT: Histologi Lecture Blok 1.4', '1.4', 'Histologi Otot, Tulang & Integumen', JSON.stringify(normalized, null, 2)));
    console.log(`Migrated ${file} -> ${normalized.length} questions`);
  }
}
