...
// Convert all 12 CBT files
const cbtList = [
  { file: '1.2-2_Biomoll DNA Repair.html', title: 'CBT: Perbaikan Kerusakan DNA', topic: 'Biologi Molekuler & DNA Repair' },
  { file: '1.2-2_Histo apoptosis dan penuaan.html', title: 'CBT: Apoptosis & Penuaan Sel', topic: 'Histologi & Regulasi Seluler' },
  { file: '1.2-2_Histo Cell Connect.html', title: 'CBT: Histologi Hubungan Antarsel', topic: 'Histologi & Junctional Complexes' },
  { file: '1.2-2_Histo Epitel.html', title: 'CBT: Histologi Jaringan Epitel & Kelenjar', topic: 'Histologi Epitel & Kelenjar' },
  { file: '1.2-2_Histo jaringan ikat.html', title: 'CBT: Histologi Jaringan Ikat', topic: 'Histologi Jaringan Ikat & Matriks' },
  { file: '1.2-2_Histo Matrix Intraseluler.html', title: 'CBT: Matriks Ekstraseluler (ECM)', topic: 'Histologi Matriks Ekstraseluler' },
  { file: '1.2-2_Histo Otot.html', title: 'CBT: Histologi Jaringan Otot', topic: 'Histologi Jaringan Otot' },
  { file: '1.2-2_Histo Saraf.html', title: 'CBT: Histologi Jaringan Saraf', topic: 'Histologi Jaringan Saraf & Glia' },
  { file: '1.2-2_Histo Siklus dan regulasi sel.html', title: 'CBT: Siklus & Regulasi Sel', topic: 'Histologi Siklus Sel & Checkpoints' },
  { file: '1.2 other_Pull up 1.html', title: 'CBT: Pull Up 1 Komprehensif Blok 1.2', topic: 'Evaluasi Komprehensif Blok 1.2 Seri A' },
  { file: '1.2 other_Pull up 2.html', title: 'CBT: Pull Up 2 Komprehensif Blok 1.2', topic: 'Evaluasi Komprehensif Blok 1.2 Seri B' },
  { file: '1.2-2_Overall CBT.html', title: 'CBT: Jackpot Kognitif Blok 1.2', topic: 'Ujian Komprehensif Akhir Blok 1.2' }
];
...
cbtList.forEach(item => {
  ...
  // Extract based on file patterns
  if (item.file === '1.2-2_Histo Cell Connect.html') {
    const m = content.match(/const quizData\s*=\s*(\[[\s\S]*?\]);/);
    if (m) {
      const raw = eval(m[1]);
      normalized = raw.map((qObj, idx) => {
        const correctIdx = qObj.options.findIndex(o => o.correct === true);
        return {
          id: idx + 1,
          q: qObj.question,
          o: qObj.options.map(o => o.text),
          a: correctIdx !== -1 ? correctIdx : 0,
          exp: qObj.explanation || '',
          src: 'Departemen Histologi FK UNSOED'
        };
      });
    }
  } else if (item.file === '1.2-2_Histo jaringan ikat.html' || item.file === '1.2-2_Histo Otot.html') {
    const m = content.match(/const quizDataSource\s*=\s*(\[[\s\S]*?\]);/);
    if (m) {
      const raw = eval(m[1]);
      normalized = raw.map((qObj, idx) => {
        const correctIdx = qObj.answers.findIndex(o => o.correct === true);
        return {
          id: idx + 1,
          q: qObj.question,
          o: qObj.answers.map(o => o.text),
          a: correctIdx !== -1 ? correctIdx : 0,
          exp: qObj.explanation || '',
          src: 'Departemen Histologi FK UNSOED'
        };
  ...
  } else if (item.file === '1.2-2_Histo Siklus dan regulasi sel.html') {
    const m = content.match(/const questions\s*=\s*(\[[\s\S]*?\]);/);
    if (m) {
      const raw = eval(m[1]);
      normalized = raw.map((qObj, idx) => {
        const correctIdx = qObj.answers.findIndex(o => o.correct === true);
        const correctItem = qObj.answers.find(o => o.correct === true);
        return {
          id: idx + 1,
          q: qObj.question,
          o: qObj.answers.map(o => o.text),
          a: correctIdx !== -1 ? correctIdx : 0,
          exp: correctItem ? (correctItem.rationale || '') : '',
          src: 'Departemen Histologi FK UNSOED'
        };
      });
    }
  } else if (item.file === '1.2-2_Histo Epitel.html') {
    const m = content.match(/const quizData\s*=\s*(\[[\s\S]*?\]);/);
    if (m) {
      const raw = eval(m[1]);
      normalized = raw.map((qObj, idx) => {
        const aIdx = qObj.options.indexOf(qObj.answer);
        return {
          id: idx + 1,
          q: qObj.question,
          o: qObj.options,
          a: aIdx !== -1 ? aIdx : 0,
          exp: qObj.review || qObj.explanation || '',
          src: 'Departemen Histologi FK UNSOED'
        };
      });
    }
  } else {
    // Standard quizData pattern with {question, options, answer, explanation}
    const m = content.match(/const quizData\s*=\s*(\[[\s\S]*?\]);/);
    if (m) {
      const raw = eval(m[1]);
      normalized = raw.map((qObj, idx) => {
        ...
        if (typeof qObj.answer === 'string') {
          aIdx = qObj.options.indexOf(qObj.answer);
          if (aIdx === -1 && /^[A-E]$/i.test(qObj.answer)) {
            aIdx = qObj.answer.toUpperCase().charCodeAt(0) - 65;
          }
        ...
        return {
          ...
          o: qObj.options,
          a: aIdx !== -1 ? aIdx : 0,
          exp: qObj.explanation || qObj.pembahasan || '',
          src: 'Bank Soal Blok 1.2 FK UNSOED'
        };
      });
    }
  }
  ...
  if (normalized.length > 0) {
    const outputHtml = getCbtTemplate(item.title, '1.2', item.topic, JSON.stringify(normalized, null, 2));
    fs.writeFileSync(filePath, outputHtml, 'utf8');
    console.log(`Migrated ${item.file} -> ${normalized.length} questions`);
  } else {
    console.error(`Failed to extract questions from ${item.file}`);
  }
});
