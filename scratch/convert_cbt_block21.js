...
.logo-wordmark span { color: #8A3B28; }
...
.meta-chip .dot { width: 6px; height: 6px; border-radius: 50%; background: #8A3B28; }
...
// Convert all 5 CBT files in 2.1
const cbtFiles = [
  { file: '2.1 CBT_22 hours warmin up.html', title: '22 HOURS WARMING UP', topic: 'Cardiorespiratory Essentials' },
  { file: '2.1 CBT_22 minute jogging.html', title: '22 MINUTE JOGGING', topic: 'Fisiologi Latihan & Pernapasan' },
  { file: '2.1 CBT_23 deadlift.html', title: '23 DEADLIFT', topic: 'Mekanika & Hemodinamika Jantung' },
  { file: '2.1 CBT_24 bench press.html', title: '24 BENCH PRESS', topic: 'Regulasi Respirasi & Gas Darah' },
  { file: '2.1 CBT_25 minute jumping jack.html', title: '25 MINUTE JUMPING JACK', topic: 'Bank Soal Komprehensif Kardiorespirasi' }
];

const basePath = path.join(process.cwd(), 'content', 'semester 2', '2.1');

cbtFiles.forEach(item => {
  const filePath = path.join(basePath, item.file);
  if (!fs.existsSync(filePath)) return;
  const raw = fs.readFileSync(filePath, 'utf8');
  
  // Extract questions array safely
  const match = raw.match(/const questions\s*=\s*(\[[\s\S]*?\]);\s*(?:let|\/\/|const|\$|function)/);
  if (match) {
    const qArrayStr = match[1];
    let questions;
    try {
      questions = eval(qArrayStr);
    } catch(e) {
      console.error('Eval error on ' + item.file, e);
      return;
    }
    
    // Normalize questions schema: q, o (array of 4-5 options), a (correct answer text), exp
    const normalized = questions.map(q => ({
      q: q.q,
      o: q.o || q.options,
      a: q.a,
      exp: q.exp || q.desc || ('<strong>Kunci Jawaban:</strong> ' + q.a)
    }));
    
    const outputHtml = getCbtTemplate(item.title, '2.1', item.topic, JSON.stringify(normalized, null, 2));
