...
// 4. Convert 1.4 Ident Anatomi_PPT ADIU 23'.html
{
  const file = '1.4 Ident Anatomi_PPT ADIU 23\'.html';
  ...
  const fc = qList.map(q => ({ front: q.q, back: q.o[q.a] + (q.exp ? ('\\n' + q.exp) : '') }));

  const html = getTypeBTemplate({
    title: 'Anatomy Mastery: 5 Regio Muskuloskeletal',
    subtitle: 'Handbook komprehensif anatomi regio Cranium, Vertebrae, Pelvis, Ekstremitas Superior, dan Ekstremitas Inferior (PPT ADIU Edition).',
    block: '1.4',
    themeColor: '#b03030',
    themeBg: 'rgba(176,48,48,.08)',
    themeBorder: 'rgba(176,48,48,.25)',
    certSubtitle: 'ANATOMI TOPOGRAFIS',
    tabs: [
      {
        id: 'cranium',
        label: 'Cranium & Vertebrae',
        title: 'Ossa Cranii, Sutura, & Columna Vertebralis',
        content: '<ul><li><strong>Sutura Cranii:</strong> Sutura coronalis (frontal-parietal), sutura sagittalis (interparietal), sutura lambdoidea (occipital-parietal).</li><li><strong>Columna Vertebralis:</strong> 7 Cervical, 12 Thoracal, 5 Lumbal, 5 Sacral (fusi), 4 Coccygis (fusi).</li></ul>'
      },
      {
        id: 'ekstremitas',
        label: 'Ekstremitas Sup & Inf',
        title: 'Tulang & Otot Ekstremitas',
        content: '<ul><li><strong>Ekstremitas Superior:</strong> Clavicula, Scapula, Humerus, Radius, Ulna, Carpal, Metacarpal, Phalanges.</li><li><strong>Ekstremitas Inferior:</strong> Os Coxae (Ilium, Ischium, Pubis), Femur, Patella, Tibia, Fibula, Tarsal, Metatarsal, Phalanges.</li></ul>'
      }
    ],
    flashcards: fc,
    questions: qList
...
// 5. Convert 1.4 Ident Biokim_eakeak fc.html
{
  const file = '1.4 Ident Biokim_eakeak fc.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  const m = content.match(/const DB\s*=\s*(\[[\s\S]*?\]);/);
  const raw = m ? eval(m[1]) : [];
  const fc = raw.map(x => ({ front: x.q, back: x.a }));
  const qList = raw.map((x, idx) => {
    const wrong = raw.filter((_, i) => i !== idx).map(w => w.a);
    wrong.sort(() => Math.random() - 0.5);
    const opts = [x.a, wrong[0], wrong[1], wrong[2]];
  ...
  const html = getTypeBTemplate({
    title: 'Biokimia Vault: Metabolisme Tulang & Mineral',
    subtitle: 'Buku saku dan flashcard 3D esensial metabolisme kalsium, hidroksiapatit, fosfat, dan kolagen tulang.',
    block: '1.4',
    themeColor: '#7a6010',
    themeBg: 'rgba(122,96,16,.09)',
    themeBorder: 'rgba(122,96,16,.28)',
    certSubtitle: 'BIOKIMIA TULANG & MINERAL',
    tabs: [
      {
        id: 'mineral-tulang',
        label: 'Matriks & Mineral',
        title: 'Komposisi Biokimia Matriks Tulang',
        content: '<p>85% fosfor dan 99% kalsium tubuh tersimpan di tulang dalam bentuk kristal <strong>Hidroksiapatit</strong> $[Ca_{10}(PO_4)_6(OH)_2]$ yang terdeposit di antara serabut kolagen tipe I.</p>'
      }
    ],
    flashcards: fc,
    questions: qList
...
// 6. Convert 1.4 Ident Biokim_kuis modul.html
{
  const file = '1.4 Ident Biokim_kuis modul.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  const m = content.match(/const questions\s*=\s*(\[[\s\S]*?\]);/);
  const raw = m ? eval(m[1]) : [];
  const qList = raw.map((q, idx) => ({
    id: idx + 1,
    q: q.text || q.q,
    o: q.options,
    a: q.answer !== undefined ? q.answer : (q.correct !== undefined ? q.correct : 0),
    exp: q.explanation || q.exp || ''
  }));
  const fc = qList.map(q => ({ front: q.q, back: q.o[q.a] + (q.exp ? ('\\n' + q.exp) : '') }));

  const html = getTypeBTemplate({
    title: 'Kuis & Modul Biokimia: Kalsium & Fosfat',
    subtitle: 'Master Study Hub 100 soal modul dan buku panduan biokimia homeostasis kalsium, fosfat, dan vitamin D.',
    block: '1.4',
    themeColor: '#136ba8',
    themeBg: 'rgba(19,107,168,.08)',
    themeBorder: 'rgba(19,107,168,.25)',
    certSubtitle: 'BIOKIMIA MODUL KELAS',
    tabs: [
      {
        id: 'vitd',
        label: 'Vitamin D & PTH',
        title: 'Jalur Aktivasi Vitamin D & Resorpsi Kalsium',
        content: '<ul><li><strong>Kulit:</strong> 7-dehidrokolesterol + UV-B $\\rightarrow$ Kolekalsiferol (Vitamin D3).</li><li><strong>Hepar:</strong> 25-hidroksilasi $\\rightarrow$ 25-OH-D3 (Kalsidiol).</li><li><strong>Ginjal:</strong> 1-alpha-hidroksilasi (distimulasi PTH) $\\rightarrow$ 1,25-(OH)2-D3 (Kalsitriol aktif).</li></ul>'
      }
    ],
    flashcards: fc,
    questions: qList
...
// 7. Convert 1.4 Ident Histologi_Osteoblast~.html
{
  const file = '1.4 Ident Histologi_Osteoblast~.html';
  const content = fs.readFileSync(path.join(dir, file), 'utf8');
  const mFc = content.match(/const flashcards\s*=\s*(\[[\s\S]*?\]);/);
  const mQ = content.match(/const quizQuestions\s*=\s*(\[[\s\S]*?\]);/);
  ...
  const html = getTypeBTemplate({
    title: 'Histologi Muskuloskeletal: Osteoblas & Jaringan Ikat',
    subtitle: 'Handbook interaktif sel-sel tulang (osteoblas, osteosit, osteoklas), kartilago, dan struktur mikro integumen.',
    block: '1.4',
    themeColor: '#4433a0',
    themeBg: 'rgba(68,51,160,.08)',
    themeBorder: 'rgba(68,51,160,.25)',
    certSubtitle: 'HISTOLOGI JARINGAN TULANG',
    tabs: [
      {
        id: 'sel-tulang',
        label: 'Sel-Sel Tulang',
        title: 'Diferensiasi Sel Tulang',
        content: '<ul><li><strong>Osteoblas:</strong> Sel pembentuk matriks organik tulang (osteoid) kaya enzim Fosfatase Alkali (ALP).</li><li><strong>Osteosit:</strong> Osteoblas matur yang terperangkap dalam lakuna dan berkomunikasi via kanalikuli.</li><li><strong>Osteoklas:</strong> Sel raksasa berinti banyak turunan monosit/makrofag di lakuna Howship yang mendegradasi matriks dengan asam dan kolagenase.</li></ul>'
      },
      {
        id: 'integumen',
        label: 'Lapisan Kulit',
        title: 'Histologi Epidermis & Dermis',
        content: '<p>5 Lapisan Epidermis (dari dalam ke luar): <strong>Stratum Basale</strong> (mitosis) $\\rightarrow$ <strong>Stratum Spinosum</strong> (desmosom) $\\rightarrow$ <strong>Stratum Granulosum</strong> (granula keratohialin) $\\rightarrow$ <strong>Stratum Lusidum</strong> (kulit tebal) $\\rightarrow$ <strong>Stratum Korneum</strong> (sel mati berkeratin).</p>'
      }
    ],
    flashcards: fc,
    questions: qList
