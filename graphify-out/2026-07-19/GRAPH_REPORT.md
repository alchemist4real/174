...
## Summary
- 4077 nodes · 6039 edges · 166 communities (158 shown, 8 thin omitted)
...
1. `4b. Workflow Logic Enhancements — `admin-workflow.js`` - 18 edges
2. `makeQ()` - 17 edges
3. `makeQ()` - 17 edges
4. `makeQ()` - 17 edges
5. `4a. Core Core Infrastructure Modifications — `admin.js`` - 15 edges
6. `makeQ()` - 15 edges
7. `3a. Document Markup Changes — `admin.html`` - 14 edges
8. `shuf()` - 14 edges
9. `makeQ()` - 14 edges
10. `makeQ()` - 14 edges
...
- `makeQ()` --calls--> `wrap`  [INFERRED]
  content/semester 2/2.5/2.5 PRAKTIKUM_PRAKTIKUM 1.html → index.html
...
Cohesion: 0.07
Nodes (48): activeTypes, AKTIV_LIST, AKTIVITAS_METs, BOUCHARD_HITUNG_KONSEP, BOUCHARD_KAT, BOUCHARD_KAT_KONSEP, DEFINISI_POOL, endSession() (+40 more)
...
Cohesion: 0.10
Nodes (45): adminAction(), adminThemeBtns, applyAdminTheme(), authMessage, authOverlay, btnGuestCleanup, currentTree, customAlert() (+37 more)
...
## Knowledge Gaps
- **1705 isolated node(s):** `graphify`, `Workflow: graphify`, `1. System Architecture & Database Schema Migrations`, `2a. Notifications Endpoint — `api/notifications.js` [NEW File]`, `2b. Invitation Links Endpoint — `api/invites.js` [NEW File]` (+1700 more)
...
- **Why does `Admin Page Feature Expansion — Unified Comprehensive Implementation Plan` connect `Admin Page Feature Expansion — Unified Comprehensive Implementation Plan` to `4. Frontend Application Logic`, `5. Verification & Testing Protocol`, `2. Backend Serverless API Implementation`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
- **What connects `graphify`, `Workflow: graphify`, `1. System Architecture & Database Schema Migrations` to the rest of the system?**
